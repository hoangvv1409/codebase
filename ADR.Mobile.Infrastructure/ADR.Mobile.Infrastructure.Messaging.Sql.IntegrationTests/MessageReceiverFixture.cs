...
namespace Infrastructure.Messaging.Sql.IntegrationTests
{
    ...
    public class given_sender_and_receiver : IDisposable
    {
        ...
        public void when_sending_message_then_receives_message()
        {
            ...
            Assert.IsTrue(this.receiver.ReceiveMessage());
            Assert.IsTrue("test message" == message.Body);
            Assert.IsTrue(message.CorrelationId == null);
            Assert.IsTrue(message.DeliveryDate == null);
        }
        ...
        public void when_sending_message_with_correlation_id_then_receives_message()
        {
            ...
            Assert.IsTrue(this.receiver.ReceiveMessage());
            Assert.IsTrue("test message" == message.Body);
            Assert.IsTrue("correlation" == message.CorrelationId);
            Assert.IsTrue(message.DeliveryDate == null);
        }
        ...
        public void when_successfully_handles_message_then_removes_message()
        {
            ...
            Assert.IsTrue(this.receiver.ReceiveMessage());
            Assert.IsFalse(this.receiver.ReceiveMessage());
        }
